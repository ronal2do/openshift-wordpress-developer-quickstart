				...
				<div class="col span_12 section-title blog-title">
					<h1>
						<?php if(is_author()){
							...
							printf( __( 'Todas as postagens de %s', NECTAR_THEME_NAME ), get_the_author() );

						} else if(is_category()) {

							printf( __( 'Categoria: %s', NECTAR_THEME_NAME ), single_cat_title( '', false ) );

						} else if(is_date()){

							if ( is_day() ) :
								printf( __( 'Arquivos diários: %s', NECTAR_THEME_NAME ), get_the_date() );

							elseif ( is_month() ) :
								printf( __( 'Arquivos mensais: %s', NECTAR_THEME_NAME ), get_the_date( _x( 'F Y', 'Arquivos Mensais (formato de data)', NECTAR_THEME_NAME ) ) );

							elseif ( is_year() ) :
								printf( __( 'Arquivos anuais: %s', NECTAR_THEME_NAME ), get_the_date( _x( 'Y', 'Arquivos anuais (formato de data)', NECTAR_THEME_NAME ) ) );

							else :
								_e( 'Arquivos', NECTAR_THEME_NAME );
